...
export default function Auth(props) {
  ...
  const formik = useFormik({
    ...
    validationSchema: flag
      ? Yup.object({
        password: Yup.string()
        .matches(/^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d]{5,}$/, 'Must be at least 5 characters and contain both alphabets and numbers')
        .required('Required'),
        cnic: Yup.string()
          .matches(/^\d{1,13}$/, "CNIC must be exactly 13 digits")
          .required("Required"),
      })
      : Yup.object({
        name: Yup.string()
    ...
    .matches(/@gmail\.com$/, 'Email must be from gmail.com')
    .required('Required'),
        password: Yup.string()
    .matches(/^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d]{5,}$/, 'Must be at least 5 characters and contain both alphabets and numbers')
    .required('Required'),
...
<Navbar/>
      ...
      <ToastContainer/>
        <div className="col-5 inner-wrapper text-center">
          <h2 className="vote-text">EMPOWER STARTUP{flag?"  LOGIN":" SIGNUP"}</h2>
